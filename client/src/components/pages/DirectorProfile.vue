                    ...
                    <v-toolbar-items class="flexbar-items">
                      ...
                      <v-layout row wrap justify-space-around>
                        ...
                        <v-flex xs11 md4 pt-3>
                          ...
                          <h3>Casting specializations</h3>
                          <v-btn outline small round color="primary" v-for="specialization in profile.specializations" :key="specialization">{{specialization}}</v-btn>
                          <v-divider></v-divider>
                          <template v-if="isSelfViewing">
                            <v-btn small to="/edit/director" block color="primary"> Edit your profile </v-btn>
                          </template>
                        </v-flex>
                      </v-layout>
                      </template>
  ...
  data () {
    return {
      ...
      headers: [{
        ...
        align: 'left',
        value: 'name'
      }, {text: 'Media type', value: 'mediaType'}, {text: 'Transmission', value: 'citiesForTransmission'}],
      noBreakdowns: 'No published breakdowns',
      toggleInfo: true,
      active: '1',
      isAgent: isAgent(),
      isActor: isActor(),
    ...
    fetchDirector (username) {
      Axios.get(`${CastingComplexAPI}/castingdirectors/?username=${username}`)
        .then((data) => {
          ...
          this.profile.companyName = profile.companyName
