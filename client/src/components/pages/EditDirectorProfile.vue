                        ...
                        <v-layout row wrap justify-space-around>
                          ...
                          <v-flex sm5 xs12>
                            ...
                            <v-select
                              ...
                              label="Casting specializations"
                              item-text="name"
                              item-value="id"
                              multiple
                              max-height="400"
                              hint="Pick your specializations"
                              persistent-hint
                            ></v-select>
                            <v-divider></v-divider>
                            <v-text-field
...
export default {
  ...
  methods: {
    ...
    mapDirectorToProps () {
      ...
      this.specializationId = profile.specializationId
    },
    submit () {
      // Trigger validation
      this.$v.$touch()
    ...
    updateDirector () {
      ...
      return Axios.put(`${CastingComplexAPI}/castingdirectors`, updatedDirector, config).then(function (response) {
        return response
      })
    ...
    getExtras (context) {
      Axios.get(`${CastingComplexAPI}/extras`)
        .then((data) => {
          this.Specializations = data.data.data['CastingSpecialization']
        }).catch((err) => {
          console.log(err)
        })
    }
